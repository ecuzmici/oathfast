...
function checkOn(files) {
  return oathfast(["check"], makeFixture(files));
}

// Unreferenced Given fails validation.
let r = checkOn({
  "GUARANTEES.md": "# GUARANTEES — x\n\nG1  A thing.  OPEN\n\n## Given\nT1  Dangling.\n",
  ".oathfast/anchors.json": JSON.stringify({ version: 1, anchors: {} }),
});
...
r = checkOn({
  ...
  ".oathfast/anchors.json": JSON.stringify({ version: 1, anchors: { G1: { kind: "trusted" } } }),
});
...
// oathfast init scaffolds a checkable repo.
const os = require("os"), fs = require("fs"), path = require("path");
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oathfast-init-"));
r = oathfast(["init"], dir);
assert(r.status === 0, "init failed");
assert(fs.existsSync(path.join(dir, "GUARANTEES.md")), "init: no GUARANTEES.md");
assert(fs.existsSync(path.join(dir, ".oathfast", "anchors.json")), "init: no anchors.json");
