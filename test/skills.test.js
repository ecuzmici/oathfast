...
// `oathfast skills` lands every one of them without touching the network
// (installation). Both must hold for `npm i -g @oathfast/cli` to be sufficient.
const fs = require("fs");
...
let r = oathfast(["skills", proj]);
assert(r.status === 0, `oathfast skills failed: ${r.stderr}`);
...
r = oathfast(["skills", proj]);
assert(r.status === 0, `second run failed: ${r.stderr}`);

// --- a skill oathfast did not install is never clobbered ---
...
// --- uninstall removes only oathfast's own decks ---
r = oathfast(["skills", proj, "--uninstall"]);
